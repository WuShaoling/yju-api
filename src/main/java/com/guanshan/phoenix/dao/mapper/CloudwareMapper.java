...
public interface CloudwareMapper {
    ...
    @Insert({
        ...
        "#{serviceId,jdbcType=VARCHAR}, #{instance,jdbcType=VARCHAR})"
    })
    ...
    @Select({
        "select",
        "id, web_socket, service_id, instance",
        "from cloudware",
        "where id = #{id,jdbcType=INTEGER}"
    })
    @ConstructorArgs({
        ...
        @Arg(column="instance", javaType=String.class, jdbcType=JdbcType.VARCHAR)
    })
